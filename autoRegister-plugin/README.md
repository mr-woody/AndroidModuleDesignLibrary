# 根据原插件是（AutoRegister）：地址：https://github.com/luckybilly/AutoRegister 进行修改：
...
1. 支持增量编译及其开关（默认开启，可通过配置`cacheEnabled = false`来关闭增量编译，[参考demo](https://github.com/luckybilly/AutoRegister/blob/master/app/build.gradle)）

2.修复gradlew build命令同时打debug和release包时，release包中注册的代码重复的问题

3.解决同时编译多个application module(点击Build -> Rebuild/Build APKs)时出现错误注册的问题

4.解决windows的兼容性问题

5.兼容java8

6.生成的注册代码不再局限于static块中，可以在任意方法(codeInsertToMethodName)中
