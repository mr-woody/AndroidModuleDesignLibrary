package com.woodys.register.plugin.extensions

import com.woodys.register.plugin.RegisterPlugin
import com.woodys.register.plugin.model.RegisterInfo
import com.woodys.register.plugin.utls.AutoRegisterHelper
...
class AutoRegisterConfig {
    ...
    AutoRegisterExtension autoRegisterExtension

    Project project

    AutoRegisterConfig(Project project,AutoRegisterExtension autoRegisterExtension) {
        this.project = project
        this.autoRegisterExtension = autoRegisterExtension
    }

    void convertConfig() {
        autoRegisterExtension.registerInfo.each { map ->
        ...
        if (autoRegisterExtension.cacheEnabled) {
            checkRegisterInfo()
        } else {
    ...
    String toString() {
        StringBuilder sb = new StringBuilder(RegisterPlugin.EXT_NAME).append(' = {')
                .append('\n  cacheEnabled = ').append(autoRegisterExtension.cacheEnabled)
                .append('\n  list = [\n')
        def size = list.size()
