...
import com.woodys.api.plugin.extension.ApiExtension
import com.woodys.api.plugin.extension.OnApiExtensionListener
import com.woodys.api.plugin.extension.Publication
import com.woodys.api.plugin.maven.PublicationManager
import com.woodys.api.plugin.maven.SourceFile
import com.woodys.api.plugin.maven.SourceSet
import org.gradle.api.GradleException
import org.gradle.api.Plugin
import org.gradle.api.Project
import org.gradle.api.publish.maven.MavenPublication
import com.woodys.api.plugin.config.Constants
import com.woodys.api.plugin.extension.Dependencies
...
class ApiPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        if (project == project.rootProject) {
            ...
            project.afterEvaluate {
                ...
                androidJarPath = com.woodys.api.plugin.utils.ModuleApiUtil.getAndroidJarPath(project, apiExtension.compileSdkVersion)

                Dependencies.metaClass.apiPublication { String value ->
                    String[] gav = com.woodys.api.plugin.utils.ModuleApiUtil.filterGAV(value)
        ...
        if (!com.woodys.api.plugin.utils.ModuleApiUtil.hasAndroidPlugin(project)) {
        ...
        project.afterEvaluate {
            com.woodys.api.plugin.utils.ModuleApiUtil.addSourceSets(project)
            ...
            if (publication.dependencies.compileOnly != null) {
                ...
                publication.dependencies.compileOnly.each {
                    if (it instanceof String && it.startsWith('api-')) {
                        String[] gav = com.woodys.api.plugin.utils.ModuleApiUtil.filterGAV(it.replace('api-', ''))
    ...
    def handleLocalJar(Project project, Publication publication) {
        ...
        File releaseJar = com.woodys.api.plugin.utils.JarUtil.packJavaSourceJar(project, publication, androidJarPath, apiExtension.compileOptions, true)
        if (releaseJar == null) {
        ...
        com.woodys.api.plugin.utils.ModuleApiUtil.copyFile(releaseJar, target)
        publication.invalid = false
        publication.useLocal = true
        publicationManager.addPublication(publication)
    }
    ...
    def handleMavenJar(Project project, Publication publication) {
        ...
        if (target.exists()) {
            if (hasModifiedSource) {
                def releaseJar = com.woodys.api.plugin.utils.JarUtil.packJavaSourceJar(project, publication, androidJarPath, apiExtension.compileOptions, true)
                if (releaseJar == null) {
                    publication.invalid = true
                    publicationManager.addPublication(publication)
                    if (target.exists()) {
            ...
            def releaseJar = com.woodys.api.plugin.utils.JarUtil.packJavaSourceJar(project, publication, androidJarPath, apiExtension.compileOptions, false)
            if (releaseJar == null) {
                publication.invalid = true
                publicationManager.addPublication(publication)
                if (target.exists()) {
            ...
            boolean equals = com.woodys.api.plugin.utils.JarUtil.compareMavenJar(project, lastPublication, releaseJar.absolutePath)
            if (equals) {
                if (target.exists()) {
                    target.delete()
                }
                ...
                releaseJar = com.woodys.api.plugin.utils.JarUtil.packJavaSourceJar(project, publication, androidJarPath, apiExtension.compileOptions, true)
                com.woodys.api.plugin.utils.ModuleApiUtil.copyFile(releaseJar, target)
                publication.useLocal = true
            }
            publication.invalid = false
